...
This document illustrates the development of the mathematical model used to calculate Net Present Values (NPV) of different agroforestry interventions promoted in Northwest Vietnam (##OWN ##CITATION). The calculation of NPV for the decision to implement coffee-based agroforesty system with the combination of teak, plum, coffee, annual crop and grass against conventional monoculture of maize is used as the illustrative example for the whole process. The same function can be used for the other interventions with corresponding input data tables for each system provided in the rest of the supplementary materials.
...
The `chance_event()` function was used to calculate the time series occurrence of risk events. This function required two main arguments 1. the probability that a particular risk event will happen in a given year and 2. the time line for the simulation. 
...
The probabilistic simulation was implemented using the `decisionSupport` function to run a Monte Carlo with 10,000 values for each of the input and response variables. These are returned as a list. A VIP score is generated for each input variable of the PLS regression.
...
## Add general output figures here
