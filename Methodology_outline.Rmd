...
`multi_EVPI()` function was used to calculate the expected value of perfect information (EVPI) for multiple variables in the computing model using the outcome table from probabilistic simulation by `decisionSupport()`function

First, we loaded the `decisionSupport` package in R. We defined sample values for all variables by generating a `make_variables` function to select from each variable in our input table. 
...
The probabilistic simulation was implemented using the `decisionSupport` function to return a Monte Carlo data table with 10,000 values for each of the input and response variables that was in the return list. A VIP score is generated for each input variable of the PLS regression.
